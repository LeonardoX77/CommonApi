...
namespace Common.Tests.Services
{
    public class SessionServiceTests : IClassFixture<TestFixture>
    {
            ...
            Session expectedEntity)
        {
            ...
            SessionDto dto = await sut.GetByPKAsync<SessionDto>(expectedEntity.Id);

            dto.Should().NotBeNull();
            dto.Should().BeOfType<SessionDto>();
            ...
            Session entity)
        {
            ...
            IQueryable<Session> filteredEntitiesMockl = default;
            repo.Setup(r => r.Get(It.IsAny<IDynamicExpression<Session>>()))
                .Returns((IDynamicExpression<Session> exp) =>
                {
                    var entities = new List<Session>() { entity };
            ...
            IPaginatedResult<SessionDto> result = await sut.Get<SessionDto, SessionQueryFilter>(filter);

            result.Should().NotBeNull();
            result.Should().BeOfType<PaginatedResult<SessionDto>>();
            ...
            [Frozen] Mock<IValidationService> validationService)
        {
            ...
            var result = await sut.Get<SessionDto, SessionQueryFilter>(filter);
            ...
            [Frozen] Mock<IValidationService> validationService)
        {
            ...
            var result = await sut.Get<SessionDto, SessionDynamicFieldsQueryFilter>(filter);
