...
namespace Common.Core.Generic.Services
{
        ...
        where TKey : struct
    {
        ...
        protected static IQueryable<TEntity> ApplyFilters(IPagination pagination, IQueryable<TEntity> query)
        {
            if (pagination != null && pagination.IsValid())
            {
                query = QueryableHelper<TEntity>.ApplySorting(query, pagination);
